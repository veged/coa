...
module.exports = function completion() {
    return this
        ...
        .act((opts, args) => {
            ...
            return Promise.resolve(complete(parsed.cmd, parsed.opts)).then(compls => {
...
function complete(cmd, opts) {
    ...
    return Promise.resolve(compls).then(complitions => {
        console.error('partialWord: %s', opts.partialWord);
        console.error('compls: %j', complitions);
        return complitions.filter(c => c.indexOf(opts.partialWord) === 0);
    });
}
