...
class Cmd extends CoaObject {
     ...
     */
    get api() {
        this._api || (this._api = (...args) => this.invoke(...args));
     ...
     *
     * @returns {COA.Arg} new argument instance
     */
    arg() {
        return new Arg(this);
    }
    ...
    _checkRequired(opts, args) {
        if(this._opts.some(opt => opt._only && Object.hasOwn(opts, opt._name))) return;
        ...
        while(i = argv.shift()) {
            ...
            if(!cmd && this._ext) {
                ...
                try {
                    ...
                    // module not found — expected for unknown subcommands
                }
    ...
    _parseOptsAndArgs(argv) {
        ...
        while(i = argv.shift()) {
            if(i !== '--' && i[0] === '-') {
                ...
                if(isRejection(res = opt._parse(argv, opts))) return res;
            ...
            while(a = i.shift()) {
                ...
                if(isRejection(res = arg._parse(a, args))) return res;
            }
        }
    ...
    _setDefaults(params, desc) {
        for(const item of desc)
            item._def !== undefined &&
                !Object.hasOwn(params, item._name) &&
    ...
    _processParams(params, desc) {
        ...
        for(const item of desc) {
            ...
            let res;
            for(const v of vals)
                if(isRejection(res = item._saveVal(params, v)))
                    return res;
        }
    ...
    _parseArr(argv) {
        const p = this._parseCmd(argv);
        if(isRejection(p)) return p.toPromise();

        const r = p.cmd._parseOptsAndArgs(p.argv);
        if(isRejection(r)) return r.toPromise();
    ...
    _do(inputPromise) {
        return Promise.resolve(inputPromise).then(input => {
            return [this._checkRequired]
                .concat(input.cmd._act || [])
                .reduce((res, act) =>
                    Promise.resolve(res).then(prev => {
                        if(isRejection(prev)) return prev.toPromise();
                        const result = act.call(input.cmd, input.opts, input.args, prev);
                        if(isRejection(result)) return result.toPromise();
                        return Promise.resolve(result).then(v =>
                            isRejection(v) ? v.toPromise() : v);
                    }),
                    undefined);
        });
    }
    ...
    run(argv) {
        ...
        const cb = code =>
            res => res?
                this._exit(res.stack || res.toString(), (Object.hasOwn(res, 'exitCode')? res.exitCode : code) || 0) :
                this._exit();

        this.do(argv).then(cb(0), cb(1));
    ...
    invoke(cmds, opts, args) {
        ...
        if(args === undefined && !Array.isArray(cmds)) {
            args = opts;
            opts = cmds;
            cmds = [];
        }
        ...
        const p = this._parseCmd(cmds);
        if(isRejection(p)) return p.toPromise();

        if(p.argv.length)
            return Promise.reject(`Unknown command: ${cmds.join(' ')}`);

        const _opts = this._processParams(opts, this._opts);
        if(isRejection(_opts)) return _opts.toPromise();

        const _args = this._processParams(args, this._args);
        if(isRejection(_args)) return _args.toPromise();
