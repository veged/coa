...
module.exports = class Opt extends CoaParam {
    ...
    act(act) {
        const opt = this;
        this._cmd.act(function(opts, args, prev) {
            if(!Object.hasOwn(opts, opt._name)) return;

            const res = act.call(this, opts, args, prev);
            if(!opt._only) return res;

            return Promise.resolve(res).then(out => this.reject({
    ...
    _checkParsed(opts) {
        return !Object.hasOwn(opts, this._name);
    }
